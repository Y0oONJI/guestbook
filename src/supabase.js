...
export async function fetchPosts() {
  ...
  return postRows.map((row) => ({ id: row.id, alias: row.nickname, emoji: row.emoji, text: row.content, likes: row.likes_count, x: row.position_x, y: row.position_y, rotate: row.rotation, liked: likedIds.has(row.id) }));
}
...
export async function createPost(post, userId) {
  ...
  if (error) throw error;
  return { id: data.id, alias: data.nickname, emoji: data.emoji, text: data.content, likes: data.likes_count, x: data.position_x, y: data.position_y, rotate: data.rotation, liked: false };
}
